...
import com.yudiz.android.util.Drawables
...
fun loadImage(
    ...
    val coilReqBuilder: LoadRequestBuilder.() -> Unit = {
        ...
        val fallback = placeholder ?: ContextCompat.getDrawable(
            iv.context,
            Drawables.ic_placeholder
        )
        error(fallback)
...
fun String.getBitmap(context: Context, listener: (Bitmap) -> Unit) {
    Coil.execute(LoadRequest.Builder(context).data(this).target {
        listener.invoke((it as BitmapDrawable).bitmap)
    }.build())
}
